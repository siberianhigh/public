   ...
   d$index = rep.int(seq_along(unique(d$CHR)), times = tapply(d$SNP,d$CHR,length))  # replcace the for loop of line 92-96 to improve efficiency
    ...
    if (!is.null(labely)) {
      def_args <- list(xaxt='n', bty='n', xaxs='i', yaxs='i', las=1, pch=20,
                       xlim=c(xmin,xmax), ylim=c(0, max(d$logp) + 0.3), # ADDED + 0.3 TO IMPROVE REPRESENTATION 
    ...
    if (!is.null(highlight)) {
        ...
        with(d.highlight, points(pos, logp, col="green2", pch=20, ...)) 
    }
    
    # Highlight second set of snps from a character vector
    if (!is.null(highlightB)) {
      if (any(!(highlightB %in% d$SNP))) warning("You're trying to highlight SNPs that don't exist in your results.")
      d.highlightB=d[which(d$SNP %in% highlightB), ]
      with(d.highlightB, points(pos, logp, col="red", pch=20, ...)) 
    }
